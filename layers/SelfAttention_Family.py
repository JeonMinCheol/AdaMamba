...
class AttentionBlock(nn.Module):
    """표준 Multi-Head Attention 블록"""
    def __init__(self, enc_in, n_heads, dropout=0.0):
        super().__init__()
        # 1. 기본 파라미터 설정
        self.enc_in = enc_in 
        self.n_heads = n_heads #
        self.d_head = enc_in // n_heads 

        # 2. Query, Key, Value를 만들기 위한 Linear 레이어
        #    하나의 큰 Linear 레이어로 한 번에 계산 후 나눕니다.
        self.qkv_layer = nn.Linear(enc_in, enc_in * 3)

        # 3. 어텐션 적용 후 결과를 합치기 위한 Linear 레이어
        self.out_layer = nn.Linear(enc_in, enc_in)
    ...
    def forward(self, x):
        ...
        # 5. Q, K, V 생성 및 헤드 분할
        qkv = self.qkv_layer(x) # [B, L, enc_in * 3]
        qkv = qkv.reshape(B, L, self.n_heads, 3 * self.d_head)
        qkv = qkv.permute(0, 2, 1, 3) # [B, n_heads, L, 3 * d_head]
        q, k, v = qkv.chunk(3, dim=-1) # 각각 [B, n_heads, L, d_head] 모양

        # 6. Scaled Dot-Product Attention 계산
        # (Q @ K.T) / sqrt(d_k)
        scores = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.d_head)
        attention_weights = F.softmax(scores, dim=-1) # [B, n_heads, L, L]

        # 7. 가중치와 Value를 곱하여 어텐션 값 계산
        attention_values = torch.matmul(self.dropout(attention_weights), v) # [B, n_heads, L, d_head]

        # 8. 분할된 헤드 다시 합치기
        attention_values = attention_values.permute(0, 2, 1, 3).reshape(B, L, self.enc_in)

        # 9. 최종 Linear 레이어 통과
        output = self.out_layer(attention_values)

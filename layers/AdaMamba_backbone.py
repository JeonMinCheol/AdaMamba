...
#         d_head = configs.d_head
#         self.mlp = nn.Sequential(
#                 nn.LayerNorm(d_model),
#                 nn.Linear(d_model, d_head),
#                 nn.GELU(),
#                 nn.Dropout(configs.head_dropout),
#                 nn.utils.weight_norm(nn.Linear(d_head, self.pred_len * self.enc_in))
#             )

#     def forward(self, summary_context):
#         return self.mlp(summary_context).view(-1, self.pred_len, self.enc_in)
...
class MeanPredictionHead(nn.Module):
    def __init__(self, configs):
        ...
        self.mlp = nn.Sequential(
            ...
            nn.GELU(),
            nn.Dropout(configs.head_dropout),
            nn.Linear(d_head, self.pred_len * self.enc_in)
        )
